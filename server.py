...
#             keep_tab_active(task_url, interval=2)
#             break
...
def wait_and_open(task_time, task_url):
    ...
    while True:
        ...
        if now >= task_time:
            # webbrowser.open(task_url)  # Opens URL in default browser
            # Start keep_tab_active in its own thread
            keep_active_thread = threading.Thread(
                target=keep_tab_active,
                args=(task_url, 2),
                daemon=True
                
            )
            print("keep_tab_active")
...
def keep_tab_active(url, interval=2):
    """
